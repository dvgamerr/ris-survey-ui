...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const row of records) {
      const [, rows] = /\((\d+)/gi.exec(row.Message)
      sales += Number(rows)
    }
    ...
    const msg = `${
      mSuccess && tSuccess === 0 ? 'OK' : 'WARN'
    } : POSDB Staging Health Check`
    const flex = lineStaging(msg, usage, task, mSuccess, max, sales, tSuccess)
