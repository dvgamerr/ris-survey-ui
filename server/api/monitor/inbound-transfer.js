...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [records] = (await pool.request().query(command)).recordsets
    ...
    const logs = `\\\\10.101.147.55\\e$\\FTP Data\\POS\\prd\\Log\\InboundTransferService`
    const file = `${logs}\\${moment()
      .add(-1, 'day')
      .format('YYYYMMDD')}_InboundTransferService.txt`
    if (fs.existsSync(file)) {
      let data = await asyncReadFile(file)
      data = data
        .toString()
        .split('\n')
        .filter((e) => /TimerElapsed Round Total/gi.test(e))
      let total = 0
      let zips = 0
      for (const line of data) {
        const [, zip, elapse] = /ZIP: (\d+) files.\((\d+)/gi.exec(line)
        zips += Number(zip)
        total += Number(elapse) / Number(zip)
      }
      const avg = Math.round(total / data.length) / 1000
      const uptime = Math.round(total / 1000 / 60 / 60)
      const msg = `${
        uptime < 20 && avg < 20 ? 'OK' : 'WARN'
      } : Inbound Transfer Health Check`
      const flex = lineInbound(
        msg,
        wait,
        error,
        date,
