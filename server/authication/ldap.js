...
const adSuffix =
  process.env.LDAP_SUFFIX || `OU=RIS,OU=UsersAccount,DC=${name},DC=co,DC=th`
...
module.exports = async (usr, pwd, filter) => {
  ...
  const asyncSearch = (suffix, options) =>
    new Promise((resolve, reject) => {
      const resolveBind = (data) =>
        client.unbind((err) => {
      ...
      client.search(suffix, options, (err, res) => {
        ...
        res.on('searchEntry', (entry) => {
          ...
          if (!filter) return resolveBind(output)
  ...
  try {
    ...
    const data = await asyncSearch(adSuffix, searchOptions(username))
  ...
  if (adUser) {
    try {
      ...
      await newClient()
      await asyncBind(account, password)
      const data = await asyncSearch(adSuffix, searchOptions(account))
      // logger.success('search entry: pass.', )
      return data
    } catch (ex) {
      // logger.error('bind: fail, ', ex.message || ex)
