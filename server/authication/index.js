...
router.get('/user', (req, res) =>
  (async () => {
    ...
    try {
      ...
      raw = raw.replace(/^bearer /gi, '')
      if (raw === 'undefined') throw new Error('user data not found.')

      const decode = decodeToken(raw)
      const data = await User.findById(decode._id, userData.join(' '))
...
router.post('/recheck', (req, res) =>
  (async () => {
    ...
    try {
      ...
      const acc = await User.findOne({ mail: user })
...
router.post('/login', (req, res) =>
  (async () => {
    ...
    if (raw && !user) {
      let IsEncode = false
      try {
        auth = new Buffer.from(raw.replace(/^basic /gi, ''), 'base64').toString(
          'utf8'
        )
        if (auth) {
          auth = /(?<usr>.*?):(?<pwd>.*)/gi.exec(auth).groups || {}
          IsEncode = true
        }
      } finally {
        /* decode but user random charector and send to server. */
      }
    ...
    const { User, UserHistory } = await db.open()
    try {
      if (!auth) throw new Error('Unauthorized (402)')
      auth.usr = auth.usr.trim().toLowerCase()

      const fullMail = !/@/g.test(auth.usr.toLowerCase())
        ? `${auth.usr.toLowerCase()}@central.co.th`
        : auth.usr.toLowerCase()
      let user = await User.findOne({
        $or: [{ mail: fullMail }, { user_name: auth.usr.toLowerCase() }],
      })
      let data = null
      try {
      ...
      const accessToken = encodeToken({ _id: user._id })
